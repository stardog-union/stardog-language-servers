...
export const makeFilterUniqueTokensByType = (typeTester: (tokenName: string) => boolean) => (
  tokens: IToken[]
) => uniq(tokens.filter(tkn => typeTester(tkn.tokenType.tokenName)).map(tkn => tkn.image));

export const isVar = (tokenName: string) => tokenName === "VAR1" || tokenName === "VAR2";
